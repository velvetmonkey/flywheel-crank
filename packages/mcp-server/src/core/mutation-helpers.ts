...
import type { MutationResult, ValidationWarning, OutputIssue } from './types.js';
...
export async function withVaultFile(
  ...
  const { vaultPath, notePath, commit, commitPrefix, section, actionDescription } = options;
  ...
  try {
    ...
    // 6. Write file back
    const finalFrontmatter = opResult.updatedFrontmatter ?? frontmatter;
